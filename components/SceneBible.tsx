...
const DOORS = [
  ...
  {
    n: 'Door Three',
    title: 'Adult ADHD Digital Biomarker - Feasibility Assessment',
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {DOORS.map((d, i) => (
            <Reveal key={d.n} delay={i * 90}>
              <a
                href={d.href}
                className="door group block h-full focus:outline-none"
                aria-label={`Open ${d.n}: ${d.title}`}
              >
                <article className="relative h-full overflow-hidden rounded-[3px] border border-rule bg-parchment-deep/30 transition-all duration-500 hover:border-gilt hover:shadow-[0_24px_50px_-24px_rgba(166,124,61,0.5)] group-focus:border-gilt group-focus:shadow-[0_24px_50px_-24px_rgba(166,124,61,0.5)]">
                  ...
                  <div className="relative z-0 flex h-full flex-col px-6 py-10 md:px-7 md:py-12">
                    ...
                    <span className="mt-8 inline-flex w-fit items-center gap-2 border border-gilt/40 px-4 py-2 font-mono text-[0.62rem] uppercase tracking-eyebrow text-gilt transition-all duration-300 group-hover:border-gilt group-hover:bg-gilt/10 group-focus:border-gilt group-focus:bg-gilt/10">
                      Enter Door
                      <span
